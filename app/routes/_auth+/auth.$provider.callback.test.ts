...
async function setupRequest({ sessionId, code = faker.string.uuid() }: { sessionId?: string; code?: string } = {}) {
  ...
  const setConnectionSessionCookieHeader = await connectionSessionStorage.commitSession(connectionSession)
